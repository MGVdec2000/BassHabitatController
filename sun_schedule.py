...
def calculate_sun_times(sun_schedule: SunSchedule, requested_date: datetime | None = None):
    ...
    :param requested_date: Date to check for sunrise and sunset. Defaults to current local datetime.
    :return: Tuple of (sunrise_datetime, sunset_datetime) in local timezone, or (None, None)
    """
    ...
    if requested_date is None:
        requested_date = datetime.now(sun_schedule.timezone)
    ...
    center = (
        1.9148 * math.sin(solar_mean_rad)
        + 0.0200 * math.sin(2 * solar_mean_rad)
        + 0.0003 * math.sin(3 * solar_mean_rad)
    )
    ...
    solar_noon = (
        2451545.0
        + solar_mean_time
        + 0.0053 * math.sin(solar_mean_rad)
    ...
    try:
        cos_hour_angle = (
            (math.sin(hour_angle_rad) - math.sin(lat_rad) * sin_delta)
...
if __name__ == "__main__":
    ...
    for hours in range(-12, 25):
        ...
        print(f"\nDate Evaluated: {target_date.strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"  Sunrise: {sunrise.strftime('%Y-%m-%d %H:%M:%S') if sunrise else 'Polar Night/Day'}")
        print(f"  Sunset:  {sunset.strftime('%Y-%m-%d %H:%M:%S') if sunset else 'Polar Night/Day'}")
